...
--
-- Public parameters (with the exception of 'nextFrameDelta' they can only be changed before calling the 'spawn()' function):
...
--		_pixels: table describing the current state of the screen and with a pointer to each pixels' object.
...
screen = {
	...
	setNextFrameDelta = function (self, _nextFrame)
		...
		for positionH=0, self.sizeH-1 do
			for positionV=0, self.sizeV-1 do
				if self._pixels[positionH][positionV].color ~= _nextFrame[positionH][positionV] then -- Checks if the new color is the same as the previous color, if it isn't adds the change to 'nextFrameDelta'
	...
	setPartialNextFrameDelta = function (self, partialNextFrameDelta)
		...
		for key, pixelState in pairs(partialNextFrameDelta) do
			...
			if self._pixels[positionH][positionV].color ~= color then -- Checks if the new color is the same as the previous color, if it isn't adds the change to 'nextFrameDelta'
				table.insert(self.nextFrameDelta, pixelState)
			end
		end
	end,
...
-- Play video by manually setting 'nextFrameDelta'
...
function update()
	if active == true then
		if playMethod == 1 then -- Play video by manually setting 'nextFrameDelta'
